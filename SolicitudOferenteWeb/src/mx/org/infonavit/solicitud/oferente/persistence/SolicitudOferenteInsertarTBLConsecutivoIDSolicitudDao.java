...
@Repository("SolicitudOferenteInsertarTBLConsecutivoIDSolicitudDao")
public class SolicitudOferenteInsertarTBLConsecutivoIDSolicitudDao extends
AbstractSolicitudExtraccionOferenteDao<SolicitudOferenteIDVo> {

	private static final Logger LOGGER = LogManager
			.getLogger(SolicitudOferenteInsertarTBLConsecutivoIDSolicitudDao.class);
...
//			return regreso;
	...
	@Override
	public boolean guardarSolicitud(final SolicitudOferenteIDVo registro) {
		
		
		int registrado = 0;
		KeyHolder keyHolder = new GeneratedKeyHolder();
		
		final String sentencia = "INSERT INTO TBL_Consecutivo_Solicitud VALUES (?,?,?,?)";
		PreparedStatementCreator psc = new PreparedStatementCreator(){
			@Override
			public PreparedStatement createPreparedStatement(Connection conexion)throws SQLException {
				PreparedStatement ps = conexion.prepareStatement(sentencia);
				ps.setString(1, ((SolicitudOferenteIDVo) registro).getAnio());
				ps.setString(2, ((SolicitudOferenteIDVo) registro).getId_estado());
				ps.setString(3, ((SolicitudOferenteIDVo) registro).getId_tipo_solicitud());
				ps.setString(4, ((SolicitudOferenteIDVo) registro).getConsecutivo());
