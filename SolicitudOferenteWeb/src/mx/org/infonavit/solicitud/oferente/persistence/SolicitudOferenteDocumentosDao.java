...
@Repository("SolicitudOferenteDocumentosDao")
public class SolicitudOferenteDocumentosDao extends AbstractSolicitudOferenteDao<SolicitudOferenteVo> {
	
	
	private static final Logger LOGGER = LogManager.getLogger(SolicitudOferenteDocumentosDao.class);
...
//	regreso.add(new SolicitudOferenteVo(resultSet.getString("id_oferente")));				
	...
	public boolean guardarOferenteSolicitud(final SolicitudOferenteVo registro) {
		...
		PreparedStatementCreator psc = new PreparedStatementCreator(){
			...
			LOGGER.debug(PREFIJO_LOG+ " Sentencia a ejecutar " + sentencia.toString());	
			registrado = this.jdbcVivienda.update(psc,keyHolder);
			LOGGER.debug(PREFIJO_LOG+ " Sentencia  " + registrado);	
			//			registrado = this.jdbcVivienda.update(psc);
			return registrado==0?true:false;
	}
	...
	public boolean updateSolicitud(SolicitudIDSolicitudOferenteVo registro) {
		int actualizados = 0;
		actualizados = this.jdbcVivienda.update("TBL_Consecutivo_Solicitud set consecutivo =" +
				" and id_estado = ",registro.getId_estado(),
				" and id_tipo_solicitud = ",registro.getId_tipo_solicitud()); 
	...
	@Override
	public int guardarOferenteSolicitudInt(SolicitudOferenteIDVo registro) {
		// TODO Apéndice de método generado automáticamente
		return 0;
	}
