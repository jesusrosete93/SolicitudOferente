...
/**
 * <p>
 * 	Clase que encapsula el ordenamiento y generación de columnas para la consula de cancelaciones Inex.
 * </p>
 * @author Adrian Casas
 *
 */
public class OrdenamientoCamposCancelacionInexUtils {
	...
	public static List<String> ordenarCampos(List<String> campos)
	{
		...
		List<String> cancelacion = new ArrayList<String>();
		...
		if( !cancelacion.isEmpty() )
		{
			...
			for( String campo : cancelacion )
			{
				camposAvaluo.put(CatalogosBusines.CATALOGO_CAMPOS_CANCELACION_AVALUO.get(campo.substring(campo.indexOf(".")+1,campo.length())).getOrdenCampo(),campo.substring(campo.indexOf(".")+1,campo.length()));
			}
			for(Integer orden : camposAvaluo.keySet())
			{
				regreso.add(CatalogosBusines.CATALOGO_CAMPOS_CANCELACION_AVALUO.get(camposAvaluo.get(orden)).getAliasTabla()+"."+CatalogosBusines.CATALOGO_CAMPOS_CANCELACION_AVALUO.get(camposAvaluo.get(orden)).getNombreColumna());
			}
		}
	 return regreso;
	}
